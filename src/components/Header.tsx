...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header className="bg-[#10141d] border-b border-[#1e2638] px-4 py-3 select-none">
      <div className="flex flex-col lg:flex-row items-center justify-between gap-4">
        {/* Brand & Stats */}
        <div className="flex items-center justify-between w-full lg:w-auto gap-4">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-rose-500/20 to-red-600/30 border border-rose-500/40 flex items-center justify-center shadow-lg shadow-rose-950/40">
          ...
          <div className="flex lg:hidden items-center gap-2 bg-rose-500/10 border border-rose-500/30 px-3 py-1.5 rounded-lg">
        ...
        <div className="flex flex-wrap items-center justify-center lg:justify-end gap-3 w-full lg:w-auto">
          ...
          <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-[#0b0e14] border border-[#242e42] text-[11px] font-mono" title="Filter timeframe using the tabs directly on the Live Drop Alerts panel">
            <Clock className="w-3.5 h-3.5 text-rose-400" />
            <span className="text-slate-400">Scan Timeframe:</span>
            <span className="text-rose-400 font-bold">{config.timeframe} Closed</span>
          </div>

          {/* Drop Threshold Config (x% Drop) */}
          <div className="flex items-center bg-[#0b0e14] border border-[#242e42] p-1 rounded-xl gap-1">
            <span className="text-[11px] font-mono text-slate-400 px-2 flex items-center gap-1">
              <Sliders className="w-3.5 h-3.5" /> Drop &ge;
            </span>
            ...
            <div className="flex items-center pl-1.5 pr-1.5 border-l border-[#242e42] ml-1">
              <span className="text-[10px] text-slate-400 mr-1 font-mono uppercase">Custom:</span>
              <input
                type="number"
                min="0.1"
                max="90"
                ...
                placeholder="%"
                className="w-14 bg-[#161c29] border border-[#2b374e] rounded px-1.5 py-0.5 text-xs font-mono text-amber-400 text-right focus:outline-none focus:border-amber-500"
              />
          ...
          <div className="flex items-center gap-2">
            ...
            <div
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-[#0b0e14] border border-[#242e42] text-[11px] font-mono text-slate-400"
              title={`Auto-scans top 100 coins strictly when each ${config.timeframe} candle closes`}
            >
              <span className="w-2 h-2 rounded-full bg-blue-500 animate-pulse"></span>
              <span className="text-slate-400 hidden sm:inline">Close Scan:</span>
