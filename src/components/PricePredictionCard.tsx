...
import { Target, TrendingUp, Settings, ExternalLink, CheckCircle2, XCircle, AlertCircle } from 'lucide-react';
...
export const PricePredictionCard: React.FC<PricePredictionCardProps> = ({
  ...
  return (
    <div className="h-full bg-slate-900/90 border border-slate-800 rounded-xl overflow-hidden shadow-xl backdrop-blur-sm flex flex-col">
      ...
      {/* Card Body */}
      <div className="p-2.5 sm:p-3 flex-1 min-h-0 flex flex-col justify-between gap-2 overflow-y-auto custom-scrollbar">
        {/* Settings Sub-Drawer */}
        {showSettings && (
          <div className="p-2.5 rounded-lg bg-slate-950/80 border border-amber-500/30 space-y-2 text-xs shrink-0">
        ...
        {feedback && (
          <div
            className={`px-2.5 py-1.5 rounded-lg text-xs flex items-center gap-2 border shrink-0 ${
        ...
        <button
          ...
          className={`w-full py-2.5 px-4 rounded-xl font-bold text-xs flex items-center justify-center gap-2 transition-all shadow-lg cursor-pointer shrink-0 ${
            calc.isValid
